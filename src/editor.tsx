...
> = (
  ...
  useEffect(() => {
    if (editorContainer.current && language) {
      ...
      const currentTheme = theme || (getWebAppTheme() === "light" ? EMonacoEditorTheme.VSDark : EMonacoEditorTheme.VS);
  ...
  useEffect(() => {
    function handleThemeChange(outTheme) {
      const currentTheme = theme || options.theme || (outTheme === "light" ? EMonacoEditorTheme.VSDark : EMonacoEditorTheme.VS);
